...
class MaskedTimmNoPoolRGBOnlyEncoder(AbstractViewEncoder):
    """
    ...
    def __init__(
        ...
        use_local_xyz: bool = False,
        use_depth: bool = False,
        use_segmentation: bool = False,
        use_pretrained_net: bool = True,
        device: str = "cuda",
    ):
        super().__init__(
        ...
        self._use_local_xyz = use_local_xyz
        ...
        in_chan = 3
        if self._use_local_xyz:
            in_chan += xyz_embed_dim
        if self._use_segmentation:
            in_chan += semantic_embedding_len
        if self._use_depth:
            in_chan += 1
        self.visual_model = timm.create_model(
            model_name=timm_class,
            pretrained=use_pretrained_net,
            in_chans=in_chan,
            num_classes=0,
        )
